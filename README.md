...
| Writing | [listener-first](skills/listener-first/SKILL.md) | Build a message from the listener's state rather than the sender's — corrects the measured sender biases first, then channel and friction, then load, and wording last. Every rule graded by evidence, with unsupported popular techniques named and excluded. |
